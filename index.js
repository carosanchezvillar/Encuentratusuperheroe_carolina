...
function rederSuperheroCard (superhero){
...
function renderSuperheroChart (superhero){
    const options ={
        ...
        data: [
            ...
            type: "pie",
            dataPoints: [
                {label: "Inteligencia", y: Number.parseInt(superhero.powerstats.intelligence)} ,
                {label: "Fuerza", y: Number.parseInt(superhero.powerstats.strength)} ,
                {label: "Velocidad", y: Number.parseInt(superhero.powerstats.speed)} ,
                {label: "Durabilidad", y: Number.parseInt(superhero.powerstats.durability)} ,
                {label: "Poder", y: Number.parseInt(superhero.powerstats.power)} ,
                {label: "Combate", y: Number.parseInt(superhero.powerstats.combat)} ,
            ]
            }
        ]
    }
...
$.ajax({
    type: "GET",
    dataType: "json",
    url: `https://www.superheroapi.com/api.php/10158604407987635/${idDelSuperHeroe}`,
}).done(function (data) {
    rederSuperheroCard(data);
